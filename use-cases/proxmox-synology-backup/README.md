...
![preview](./proxmox-synology-backup.webp)
...
Please refer to [Gotify](https://gotify.net)'s official documentation for installing and configuring. I installed it in a Proxmox LXC by using the helper script provided by [Community-Scripts](https://community-scripts.github.io/ProxmoxVE).
...
Since we will use MQTT to publish last backup date to Home Assistant, you need a `MQTT broker` (such as Mosquitto) already up&running. If not, you could install and configure Eclipse Mosquitto as a standalone service in a Proxmox LXC (again you can use the helper script provided by [Community-Scripts](https://community-scripts.github.io/ProxmoxVE)) or directly from Home Assistant via the official Add-On. 
...
1. **Import Flow:**
   - Import the flow into Node-RED using the [Gotify Backup Notifications](/downloads/GotifyBackup_Notifications.json) JSON provided 
